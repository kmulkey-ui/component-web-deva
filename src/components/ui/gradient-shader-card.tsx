...
import { useRef, useMemo, useImperativeHandle, forwardRef, useState } from "react"
...
const fragmentShader = `
  ...
  vec3 hexToRgb(float r, float g, float b) { return vec3(r/255.0, g/255.0, b/255.0); }
  ...
  vec3 multiColorGradient(float t) {
    ...
    return mix(colors[index], colors[index + 1], smoothstep(0.0, 1.0, localT));
  }
...
// ─── R3F mesh (must be inside <Canvas>) ──────────────────────────────────────
...
const GrainyGradient = forwardRef<any, GrainyGradientProps>(
  ({ ripples = [], onTimeUpdate }, ref) => {
    ...
    useImperativeHandle(ref, () => ({
      material, uniforms, getCurrentTime: () => uniforms.iTime.value,
    }))
    ...
    useFrame((state) => {
      ...
      const active = ripples.filter(r => t - r.startTime < 2.0)
      const positions: number[] = []
      const times: number[] = []
      for (let i = 0; i < Math.min(active.length, 10); i++) {
        const r = active[i]
        positions.push((r.x / 800) * 2 - 1, 1 - (r.y / 600) * 2)
        times.push(t - r.startTime)
      }
...
// ─── Public card wrapper (self-contained) ────────────────────────────────────

interface GradientShaderCardProps {
  className?: string
  children?: React.ReactNode
}

export function GradientShaderCard({ className = "", children }: GradientShaderCardProps) {
  ...
  return (
    <motion.div
      ...
      style={{
        borderRadius: "48px",
        filter: "drop-shadow(0px 51px 51px rgba(0,0,0,0.09)) drop-shadow(0px 13px 28px rgba(0,0,0,0.1))",
      }}
